...
public sealed class WwiseRiffVorbis
{
    ...
    private readonly long _riffSize;
    ...
    public void GenerateOgg(Stream outputStream)
    {
        ...
        var blocksize0 = 1u << _blocksize0Pow;
        var blocksize1 = 1u << _blocksize1Pow;
        long granulePos = 0;
        ...
        while (offset < _dataOffset + _dataSize)
        {
            ...
            var isLastPacket = nextOffset >= _dataOffset + _dataSize;
            ...
            if (_modPackets)
            {
                ...
                var modeNumber = bitReader.ReadBits(modeBits);
                ogg.WriteBits(modeNumber, modeBits);

                // IN: remaining bits of first byte
                var remainder = bitReader.ReadBits(8 - modeBits);
                ...
                if (modeBlockflag[modeNumber])
                {
                    ...
                    if (nextOffset + packetHeaderSize <= _dataOffset + _dataSize)
                    {
                        ...
                        if (nextPacket.Size > 0)
                        {
                            ...
                            var nextModeNumber = nextBitReader.ReadBits(modeBits);
    ...
    private void GenerateOggHeader(BitOggStream ogg, out bool[] modeBlockflag, out int modeBits)
    {
        ...
        var vendor = $"converted from Audiokinetic Wwise by ww2ogg {Version}";
        ...
        else
        {
            ...
            var loopStart = $"LoopStart={_loopStart}";
            var loopEnd = $"LoopEnd={_loopEnd}";

            ogg.WriteBits((uint) loopStart.Length, 32);

            foreach (var c in loopStart)
            {
                ogg.WriteBits(c, 8);
            }
        ...
        // Codebook count
        var codebookCountLess1 = setupReader.ReadBits(8);
        ...
        else
        {
            for (uint i = 0; i < codebookCount; i++)
            {
                var codebookId = setupReader.ReadBits(10);
                ...
                catch (InvalidCodebookIdException)
                {
                    if (codebookId == 0x342)
                    {
                        var codebookIdentifier = setupReader.ReadBits(14);
    ...
    private void WriteVorbisPacketHeader(BitOggStream ogg, byte type)
    {
        ...
        foreach (var b in VorbisBytes)
        {
            ogg.WriteBits(b, 8);
        }
    }

    private void RebuildSetupData(
        BitReader reader,
        BitOggStream ogg,
        uint codebookCount,
        out bool[] modeBlockflag,
        out int modeBits)
    {
        // Floor count
        var floorCountLess1 = reader.ReadBits(6);
        ...
        // Residue count
        var residueCountLess1 = reader.ReadBits(6);
        ...
        // Mapping count
        var mappingCountLess1 = reader.ReadBits(6);
        ...
        // Mode count
        var modeCountLess1 = reader.ReadBits(6);
        ...
        {
            var blockFlag = reader.ReadBits(1);
    ...
    {
        var floor1Partitions = reader.ReadBits(5);
        ...
        {
            var floor1PartitionClass = reader.ReadBits(4);
        ...
        for (uint j = 0; j <= maximumClass; j++)
        {
            ...
            {
                var subclassBookPlus1 = reader.ReadBits(8);
                ogg.WriteBits(subclassBookPlus1, 8);
                var subclassBook = (int) subclassBookPlus1 - 1;
        ...
        var floor1MultiplierLess1 = reader.ReadBits(2);
        ogg.WriteBits(floor1MultiplierLess1, 2);

        var rangebits = reader.ReadBits(4);
        ...
        for (uint j = 0; j < floor1Partitions; j++)
        {
            ...
            {
                var x = reader.ReadBits((int) rangebits);
    ...
    {
        var residueType = reader.ReadBits(2);
        ...
        var residueBegin = reader.ReadBits(24);
        var residueEnd = reader.ReadBits(24);
        var residuePartitionSizeLess1 = reader.ReadBits(24);
        var residueClassificationsLess1 = reader.ReadBits(6);
        var residueClassbook = reader.ReadBits(8);
        ...
        for (uint j = 0; j < residueClassifications; j++)
        {
            for (var k = 0; k < 8; k++)
            {
                ...
                {
                    var residueBook = reader.ReadBits(8);
    ...
    private void RebuildMapping(BitReader reader, BitOggStream ogg, uint floorCount, uint residueCount)
    {
        ...
        var submapsFlag = reader.ReadBits(1);
        ...
        if (submapsFlag != 0)
        {
            var submapsLess1 = reader.ReadBits(4);
            submaps = submapsLess1 + 1;
            ogg.WriteBits(submapsLess1, 4);
        }

        var squarePolarFlag = reader.ReadBits(1);
        ogg.WriteBits(squarePolarFlag, 1);

        if (squarePolarFlag != 0)
        {
            var couplingStepsLess1 = reader.ReadBits(8);
            ...
            {
                var magnitude = reader.ReadBits(couplingBits);
                var angle = reader.ReadBits(couplingBits);
        ...
        var mappingReserved = reader.ReadBits(2);
        ...
        if (submaps > 1)
        {
            ...
            {
                var mappingMux = reader.ReadBits(4);
        ...
        {
            var timeConfig = reader.ReadBits(8);
            ogg.WriteBits(timeConfig, 8);

            var floorNumber = reader.ReadBits(8);
            ...
            var residueNumber = reader.ReadBits(8);
    ...
    private void GenerateOggHeaderWithTriad(BitOggStream ogg)
    {
        ...
        var type = setupReader.ReadBits(8);
